...
use bevy::render::render_resource::{
    ...
    BufferUsages, PipelineCache, RenderPipelineDescriptor, ShaderStages, SpecializedMeshPipeline,
...
impl Plugin for ShaderPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugin(PlanetMaterialPlugin);
    }
}
...
// extract the `PlanetMaterial` component into the render world
fn extract_custom_material(
    mut commands: Commands,
    mut previous_len: Local<usize>,
    mut query: Query<Entity, With<PlanetMaterial>>,
) {
    let mut values = Vec::with_capacity(*previous_len);
    for entity in query.iter_mut() {
        values.push((entity, (PlanetMaterial,)));
    }
    *previous_len = values.len();
    commands.insert_or_spawn_batch(values);
}

// add each entity with a mesh and a `PlanetMaterial` to every view's `Transparent3d` render phase using the `CustomPipeline`
...
fn queue_custom(
    ...
    material_meshes: Query<(Entity, &MeshUniform, &Handle<Mesh>), With<PlanetMaterial>>,
...
impl FromWorld for CustomPipeline {
    fn from_world(world: &mut World) -> Self {
        ...
        let shader = asset_server.load("shaders/planet_shader.frag.wgsl");
